...
void MainGui::render() {
  ...
  {
    if (ImGui::Button("Open Bin File Dialog")) {
      IGFD::FileDialogConfig config;
      config.path = ".";
      ImGuiFileDialog::Instance()->OpenDialog(
          "ChooseFileDlgKey", "Choose Bin File", ".bin", config);
    }
  ...
  {
    fs::path filepath{"F:/tmp/20240328 ex vivo 255/120114PAUS.bin"};
  ...
  //  const size_t aline_size = static_cast<size_t>(m_rf.rows());
  //  if (aline_size > 0) {
  ...
  //    auto kernel = arpam::signal::firwin2(65, freq, gain);

  //    arpam::recon::FIRFilter fir_filter(aline_size, {numtaps, freq, gain});
  //    arpam::recon::EnvelopeDetection envelope_detection(aline_size);

  //    arpam::recon::LogCompression log_compress(aline_size, dynamic_range);

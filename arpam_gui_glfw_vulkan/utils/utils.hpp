...
void oneTimeSubmit(vk::Device const &device, vk::CommandPool const &commandPool,
...
void copyToDevice(vk::Device const &device,
                  vk::DeviceMemory const &deviceMemory, T const *pData,
                  size_t count, vk::DeviceSize stride = sizeof(T)) {
  assert(sizeof(T) <= stride);
  uint8_t *deviceData =
      static_cast<uint8_t *>(device.mapMemory(deviceMemory, 0, count * stride));
  if (stride == sizeof(T)) {
...
template <class T>
void copyToDevice(vk::Device const &device,
                  vk::DeviceMemory const &deviceMemory, T const &data) {
...
struct BufferData {
  BufferData(vk::PhysicalDevice const &physicalDevice, vk::Device const &device,
  ...
  void clear(vk::Device const &device) {
    device.destroyBuffer(
        buffer); // to prevent some validation layer warning, the Buffer needs
                 // to be destroyed before the bound DeviceMemory
  ...
  template <typename DataType>
  void upload(vk::Device const &device, std::vector<DataType> const &data,
  ...
  void upload(vk::PhysicalDevice const &physicalDevice,
              vk::Device const &device, vk::CommandPool const &commandPool,
              vk::Queue queue, std::vector<DataType> const &data,
              size_t stride) const {
    ...
    vk::su::BufferData stagingBuffer(physicalDevice, device, dataSize,
                                     vk::BufferUsageFlagBits::eTransferSrc);
    copyToDevice(device, stagingBuffer.deviceMemory, data.data(), data.size(),
                 elementSize);
...
struct ImageData {
  ImageData(vk::PhysicalDevice const &physicalDevice, vk::Device const &device,
  ...
  void clear(vk::Device const &device) {
    ...
    device.destroyImage(image); // the Image should to be destroyed before the
                                // bound DeviceMemory is freed
    device.freeMemory(deviceMemory);
  }
...
struct SwapChainData {
  SwapChainData(vk::PhysicalDevice const &physicalDevice,
                vk::Device const &device, vk::SurfaceKHR const &surface,
...
public:
  PixelsImageGenerator(vk::Extent2D const &extent, size_t channels,
...
struct TextureData {
  ...
  void setImage(vk::Device const &device,
    ...
    void *data =
        needsStaging
            ? device.mapMemory(
                  stagingBufferData->deviceMemory, 0,
                  device.getBufferMemoryRequirements(stagingBufferData->buffer)
                      .size)
            : device.mapMemory(
                  imageData->deviceMemory, 0,
                  device.getImageMemoryRequirements(imageData->image).size);
    imageGenerator(data, extent);
    device.unmapMemory(needsStaging ? stagingBufferData->deviceMemory
                                    : imageData->deviceMemory);
...
vk::Pipeline createGraphicsPipeline(
    ...
    vk::PipelineLayout const &pipelineLayout, vk::RenderPass const &renderPass);
...
vk::RenderPass createRenderPass(
    vk::Device const &device, vk::Format colorFormat, vk::Format depthFormat,
...
void updateDescriptorSets(
    ...
    std::vector<std::tuple<vk::DescriptorType, vk::Buffer const &,
        ...
        &bufferData,
    vk::su::TextureData const &textureData, uint32_t bindingOffset = 0);
void updateDescriptorSets(
    vk::Device const &device, vk::DescriptorSet const &descriptorSet,
    std::vector<std::tuple<vk::DescriptorType, vk::Buffer const &,
                           vk::DeviceSize, vk::BufferView const &>> const

...
struct fftw_engine_c2r_1d {
  ...
  explicit fftw_engine_c2r_1d(size_t n)
      : n(n), real(fftw_alloc_real(n)), complex(fftw_alloc_complex(n / 2 + 1)),
        plan(fftw_plan_dft_c2r_1d(static_cast<int>(n), complex, real,
                                  FFTW_ESTIMATE)) {}
  ~fftw_engine_c2r_1d() {
    fftw_destroy_plan(plan);
    fftw_free(real);
...
struct fftw_engine_half_cx_1d {
  ...
  explicit fftw_engine_half_cx_1d(size_t n)
      : n(n), real(fftw_alloc_real(n)), complex(fftw_alloc_complex(n / 2 + 1)),
        plan_f(fftw_plan_dft_r2c_1d(static_cast<int>(n), real, complex,
                                    FFTW_ESTIMATE)),
        plan_b(fftw_plan_dft_c2r_1d(static_cast<int>(n), complex, real,
                                    FFTW_ESTIMATE)) {}
...
struct fftw_engine_1d {
  ...
  explicit fftw_engine_1d(size_t n)
      : n(n), in(fftw_alloc_complex(n)), out(fftw_alloc_complex(n)),
        plan_f(fftw_plan_dft_1d(static_cast<int>(n), in, out, FFTW_FORWARD,
                                FFTW_ESTIMATE)),
        plan_b(fftw_plan_dft_1d(static_cast<int>(n), out, in, FFTW_BACKWARD,
                                FFTW_ESTIMATE)) {}
